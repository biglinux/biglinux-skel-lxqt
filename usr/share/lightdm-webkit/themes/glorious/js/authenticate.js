class Authenticate {
	...
	_returnRandomErrorMessages() {
		const errorMessages = [
			...
			'Está digitando errado!',
			'Você não anota suas senhas?',
			'Por que você é desse jeito?',
			'Isso vai se autodestruir em 5 segundos!',
			'Uhhh... tem certeza que sabe o que está fazendo?',
			'“Você erra 100% das tacadas que não dá – Wayne Gretzky – Michael Scott”',
			'Saia daí que vai explodir!',
	...
	_returnRandomSuccessfulMessages() {
		const errorMessages = [
			'Sucesso na autenticação! Fazendo login!',
			'Fazendo Login! Muito bem!',
			'Hum... Parece tudo bem!',
			'Está bem! Você está entrando',
			'Você é a razão do sorriso de alguém.',
			'Finalmente, alguém com um bom QI!',
			'Por favor, não vá fazer besteira hoje.'
		];
		return errorMessages[Math.floor(Math.random() * errorMessages.length)];
	}
